...
class LANG extends API {
    ...
    function __construct($API) {
        ...
        if ($this->API->CONFIG['static_language']) {
            $langfiles = explode(',', $this->API->CONFIG['static_language']);
    ...
    public function load($language = 'en', $file = '') {
        if (!$file)
            ...
            $this->parse_langfile($this->API->CONFIG['ROOT_PATH'] . $file, $language);
    }
