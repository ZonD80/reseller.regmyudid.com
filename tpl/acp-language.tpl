...
<head>
    ...
    <title>Admicp simple - Language helper tools</title>
    {literal}
        ...
        <form method="POST"><input type="hidden" name="do" value="gensave"/>
            ...
            <table border="1" width="100%" id="langtable">
                ...
                {foreach from=$data item=d}
                    <tr id="{$d.lkey}-{$d.ltranslate}"><td><input type="text" required="required" name="key[{$d.lkey|htmlspecialchars}][{$d.ltranslate|htmlspecialchars}]" value="{$d.lkey|htmlspecialchars}" maxlength="255"/></td><td>{$d.ltranslate}</td><td><textarea rows="2" cols="100" name="val[{$d.lkey|htmlspecialchars}][{$d.ltranslate|htmlspecialchars}]">{$d.lvalue|htmlspecialchars}</textarea></td><td><a onclick="return ajaxdel('{$d.lkey|htmlspecialchars}', '{$d.ltranslate|htmlspecialchars}');" href="javascript://">Delete</a></td></tr>
                        {/foreach}
            </table>
            {$pagercode}
